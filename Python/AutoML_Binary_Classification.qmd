...
In this context, we fit and test the models similarly to the previous approach, but we specifically evaluate and **select the best model based on its ability to maintain a low false negative rate**.
...
class AutoML:
    ...
    def evaluate_models(self):
        ...
        # Determine the best model
        if self.evaluation_metric == 'false_negatives':
            sorted_results = sorted(results.items(), key=lambda x: x[1]['false_negatives'])
        else:
            sorted_results = sorted(results.items(), key=lambda x: (x[1]['accuracy'], x[1][self.evaluation_metric]), reverse=True)
...
auto_ml = AutoML(reduced_df, target_column='thirtydaymortality', balance='imbalanced', evaluation_metric='false_negatives')
best_model_name, best_model_metrics = auto_ml.evaluate_models()
...
Time taken = 34.53 sec

Best score = 0.879 using params: OrderedDict(\[('ccp_alpha', 0.001131086677421503), ('criterion', 'entropy'), ('max_depth', 24), ('max_features', 'log2'), ('min_samples_leaf', 3), ('min_samples_split', 3)\])

**Test accuracy = 0.882**

![](images/clipboard-3184518635.png)

![](images/clipboard-2704166000.png)

Evaluating RandomForest...

Time taken = 882.46 sec
Best score = 0.97 using params: OrderedDict(\[('criterion', 'entropy'), ('max_depth', 27), ('max_features', 'sqrt'), ('min_samples_leaf', 2), ('min_samples_split', 3), ('n_estimators', 82)\])
**Test accuracy = 0.956**

![](images/clipboard-189649097.png)

![](images/clipboard-998271002.png)

Evaluating XGBoost...

Time taken = 481.13 sec
Best score = 0.973 using params: OrderedDict(\[('colsample_bytree', 0.8636816590466002), ('learning_rate', 0.0645354435221669), ('max_depth', 29), ('n_estimators', 108), ('subsample', 0.9220560088509978)\])
**Test accuracy = 0.952**

![](images/clipboard-388346725.png)

![](images/clipboard-370685391.png)

Evaluating LightGBM...

Time taken = 234.32 sec
Best score = 0.958 using params: OrderedDict(\[('colsample_bytree', 0.5133739717626367), ('learning_rate', 0.1941545477346783), ('max_depth', 24), ('n_estimators', 131), ('subsample', 0.8487299825534435)\])
**Test accuracy = 0.941**

![](images/clipboard-2732899064.png)

![](images/clipboard-2798390817.png)

[**Best Model: DecisionTree**]{.underline}
Accuracy: 0.8822077718583516
Precision: 0.9921306122285661
Recall: 0.8822077718583516
F1 Score: 0.9320824551973529
False Negatives: 51
Confusion Matrix:
None

::: {.notebox .lightbulb data-latex="lightbulb"}
**Upon fine-tuning to context: Minimising False Negatives** The answer we obtained is actually different from our original best-choice model which was Random Forest. Now It is Decision Tree instead.
:::
